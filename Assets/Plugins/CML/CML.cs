/*
	Custom Markup Language (CML) V1.0 Copyright @ myBad Studios 2012
	http://www.mybadstudios.com
	
	CML is created in the hope that it will be useful and is released into the
	public domain, granting you a royalty free, irrevokable, worldwide license
	to use CML in your own works subject to the single condition stipulated below.
	
	You are free to expand upon this code and create derivative works but
	in using CML you agree that this first comment block, including but not
	limited to the copyright notice and examples, will always be included in this
	and any derived work without being altered in any way. Modifying or omitting
	this comment block is in violation of the terms of use of CML and nullifies
	your right of use hereof, and by proxy any derived works created by you or
	any other party.
	
	CML was created to provide a simple format for storing, retrieving and navigating
	any configurable set of data and allows the use of any custom dataset that
	might be required. Data sets can be classified by type, and datablocks of
	such sets can be placed in your file in any order. You are not limited to
...
/* ------------------------------------------------------------------------
	...
	virtual public cmlData Copy(bool include_id = true)
...
public class cmlData : IDisposable {
	public string						data_type;
	public Dictionary<string, string>	defined;
	public List<string>					data;	
	...
	static public implicit operator bool(cmlData v) { return null != v; }
...
public class CML : IDisposable {
	const float __CMLVersion = 2.0f;
