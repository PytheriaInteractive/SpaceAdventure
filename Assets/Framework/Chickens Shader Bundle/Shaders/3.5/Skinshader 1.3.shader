Shader "Chickenlord/3.5/Skin/SSS Skinshader 1.31"{
	Properties {
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surf BlinnPhongSSSLayer3 nolightmap fullforwardshadows addshadow novertexlights
		#pragma target 3.0
		...
		inline fixed4 LightingBlinnPhongSSSLayer3 (SurfaceOutputPS s, fixed3 lightDir, fixed3 viewDir, fixed atten)
		{
			...
			c2.a = s.Alpha + _LightColor0.a * _SpecColor.a * atten;
			...
			fixed4 c;
			c.rgb = (s.Albedo * float3(_LightColor0.r * dr, _LightColor0.g * dg,_LightColor0.b * db) + _LightColor0.rgb * _SpecColor.rgb * spec) * (atten * 2);
