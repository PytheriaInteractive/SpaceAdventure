...
public enum ReflectionRenderResolution
{
    Low,
    Medium,
    High
...
public class CSB_ReflectionFx : MonoBehaviour 
{
	...
	private RenderTexture CreateTextureFor(Camera cam) 
	{
        ...
        if (ColorDepth == RenderColorDepth.RGB565)
        {
            rtFormat = RenderTextureFormat.RGB565;
        }
		if (!SystemInfo.SupportsRenderTextureFormat (rtFormat))
			rtFormat = RenderTextureFormat.Default;

        float rtSizeMul = 0.75f;
        switch (resolution)
        {
            case ReflectionRenderResolution.Low:
                rtSizeMul = 0.25f;
                break;
            case ReflectionRenderResolution.Medium:
                rtSizeMul = 0.5f;
                break;
            default:
                break;
	...
	private void RenderReflectionFor (Camera cam, Camera reflectCamera) 
	{
        ...
        {
            reflectCamera.RenderWithShader(replacementShader, ReplacementTag);
        }
        else
        {
