...
inline half CheckSame (float2 uvs)
{
	float depthA  = (UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture,uvs)));
	float depthB  = (UNITY_SAMPLE_DEPTH(tex2D(_SecRealDepth,uvs)));
...
half4 frag( v2f i ) : COLOR
{
	...
	half blending = CheckSame(i.uv[0]);
	half3 blurred = 0;
...
// -- DepthTextureCopy
	...
	half4 frag( v2f i ) : COLOR
	{
		...
		half3 ret = c*_direct+blurred;
		half maxV = max(max(ret.r,max(ret.g,ret.b)),0.004);
		ret /= maxV;
		maxV /= 8;
		return float4(ret,maxV);
	}
	ENDCG
	}
	...
	// -- CopyPass
		Pass {
			...
			fixed4 frag (v2f i) : COLOR
			{
				return tex2D(_CameraDepthTexture,i.texcoord[0]);
				//return SecDepth;
			}
			ENDCG 
		...
		Pass {
			...
			inline half CheckSame2 (half4 n, half depth)
			{
				// difference in normals
				half2 diff = abs(n.xy - nn.xy);
				half sn = (diff.x + diff.y) < 0.1;
			...
			fixed4 frag (v2f i) : COLOR
			{
				return Linear01Depth (UNITY_SAMPLE_DEPTH(tex2D(_MainTex,i.texcoord[0])));
				//return SecDepth;
			}
			ENDCG 
		...
		Pass {
			...
			fixed4 frag (v2f i) : COLOR
			{
				float od = Linear01Depth (UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture,i.texcoord[0])));
				float sd = Linear01Depth (UNITY_SAMPLE_DEPTH(tex2D(_SecRealDepth,i.texcoord[0])));
				fixed same = od == sd? 1 : 0;
