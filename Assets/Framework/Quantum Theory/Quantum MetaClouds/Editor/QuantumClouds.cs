...
public class QuantumClouds : EditorWindow {
	...
	{		
		for(int x=0;x<selection.Length;x++)
			{
			if(selection[x].renderer.sharedMaterial.shader.name.Contains("QuantumMetaCloud"))
				{
				selection[x].renderer.sharedMaterial.SetColor("_AmbientColor",AmbientColor);
				selection[x].renderer.sharedMaterial.SetColor("_SunColor",SunColor);
				selection[x].renderer.sharedMaterial.SetFloat("_CloudContrast",Contrast);
				selection[x].renderer.sharedMaterial.SetFloat("_Bias",Bias);
				selection[x].renderer.sharedMaterial.SetFloat("_AlphaCutoff",Opacity);
				}			
			}
	}
	...
	void GetMaterialValues(GameObject[] selection)
	{
		...
		{
		SunColor = selection[0].renderer.sharedMaterial.GetColor("_SunColor");
		AmbientColor = selection[0].renderer.sharedMaterial.GetColor("_AmbientColor");
		Contrast = selection[0].renderer.sharedMaterial.GetFloat("_CloudContrast");
		Bias = selection[0].renderer.sharedMaterial.GetFloat("_Bias");
		Opacity = selection[0].renderer.sharedMaterial.GetFloat("_AlphaCutoff");
					}
